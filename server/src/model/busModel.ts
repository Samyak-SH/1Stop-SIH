...
export async function getNearestStopsModel(
...
export async function getCommonRoutesModel(
  sourceId: string,
  ...
  // Intersection
  const common: string[] = routes1
    .filter((r1) => routes2.some((r2) => r1.routeNumber === r2.routeNumber))
    .map((r) => r.routeNumber);
